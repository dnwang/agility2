...
public class FlowLayout extends ViewGroup {
    ...
    public FlowLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        try {
            String[] space = getTag().toString().split(",");
            mHorizontalSpacing = UIUtils.dip2px(context, Integer.parseInt(space[0]));
            mVerticalSpacing = UIUtils.dip2px(context, Integer.parseInt(space[1]));
        } catch (Exception e) {
            mHorizontalSpacing = 0;
            mVerticalSpacing = 0;
        }
